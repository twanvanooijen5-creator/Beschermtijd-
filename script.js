function berekenTijd() {
  ...
  const levensJaren = (totaalUren * levensverwachting) / (24 * 365);
  ...
  const output = `
    ...
    <p>In een gemiddeld mensenleven van ${levensverwachting} jaar betekent dat 
    ongeveer <strong>${levensJaren.toFixed(1)} jaar</strong> aan schermtijd!</p>
  `;
...
document.getElementById("downloadBtn").addEventListener("click", function () {
  ...
  if (!isMobile) {
    ...
    const ticketHTML = `
      ...
      <head>
          ...
          <style>
            ...
            .boekenleggerContainer { border: 2px dashed #555; margin: 20px auto 30px auto; width: 18cm; height: 5cm; background: linear-gradient(to right, #ff0000 ${percentageLeven.toFixed(1)}%, #4CAF50 ${percentageLeven.toFixed(1)}%); color: white; font-size: 2em; font-weight: bold; display: flex; align-items: center; justify-content: center; text-align: center; letter-spacing: 4px; text-shadow: 1px 1px 3px rgba(0,0,0,0.6);}
    ...
    tekstregels.forEach((regel, i) => doc.text(regel, centerX, 45 + i * 8, { align: "center" }));
    ...
    doc.text(lines, centerX, startY + 50, { align: "center" });
